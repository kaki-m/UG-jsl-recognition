...
2022/12/25
特徴値生成コード3
3枚の画像間で座標がどのくらい動いたのかを保持する
features3では3枚のフレームを使用して、1->2,2->3の間の変化量を特徴量に加える
特徴量は全部で[距離、角度、1->2,2->3]となる
'''
import os
import pandas as pd
...
for f in tqdm(srcFiles):
    ...
    nextFileName = Name + '_' + hand_type + '_' + str(file_num + 1) + '.csv'
    nextnextFileName = Name + '_' + hand_type + '_' + str(file_num + 2) + '.csv'
    #次のフレームのファイル名を得るために変数に入れる
    try:
        nextdata = np.loadtxt('./data/' + Name + '/' + srcdir + '/' + nextFileName,dtype='float32',delimiter=',')
        nextnextdata = np.loadtxt('./data/' + Name + '/' + srcdir + '/' + nextnextFileName,dtype = 'float32', delimiter = ',')
    ...
    for i in range(21):#すべてのランドマークに対して一枚目と二枚目の変化(variation)を計算する
        ...
        #次の次のフレームとの差分を計算して入れる
        dist3.append(nextnextdata[i][0] - data[i][0])
        mark3.append("variation2X_" + str(i))
        dist3.append(nextnextdata[i][1] - data[i][1])
        mark3.append("variation2Y_"+str(i))
        dist3.append(nextnextdata[i][2] - data[i][2])
        mark3.append("variation2Z_"+str(i))
