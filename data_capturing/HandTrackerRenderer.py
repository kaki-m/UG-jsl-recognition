...
class HandTrackerRenderer:
        ...
        if self.tracker.use_lm:
            ...
            if hand.lm_score > self.tracker.lm_score_thresh:
                ...
                if self.tracker.use_gesture and self.show_gesture:
                    if hand.gesture == "ONE" or hand.gesture == "TWO":
                        cv2.putText(self.frame, "writing", (100, self.tracker.resolution[1]-100), 
    ...
    def drawZcoordinates(self, frame, hands, coordinates):
        for h in hands:
            """
            for j, c in enumerate(coordinates):
                cv2.putText(frame,
                        text = str(format(coordinates[j, 2], '.2f')),
                        org=(h.landmarks[j, 0], h.landmarks[j, 1]+15),
                        ...
                        """
            cv2.putText(frame,
                        text = str(format(coordinates[8, 2], '.2f')),
                        org=(h.landmarks[8, 0], h.landmarks[8, 1]+15),
                        fontFace = cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale = 2,
                        color = (0, 255, 0),
                        thickness = 2
    ...
    def waitKey(self, delay=1):
        ...
        #ここで表示するビデオ映像はcollect_data内で表示したほうがいいかもしれない
        #cv2.imshow("Hand tracking", self.frame)
        #cv2.imshow("Hand tracking", cv2.flip(self.frame, 1))

        '''
        ここを自分で作り替えてエンターキーを押したときにframeを画像として保存するようにしようと思う
        2022/11/2
        '''
