...
class HandTracker:
    ...
    def __init__(self, input_src=None,
        ...
        if self.input_type == "rgb":
            ...
            if self.xyz:
                ...
                self.q_spatial_depth_in = self.device.getInputQueue("spatial_calc_depth_in")

                self.q_depth_out = self.device.getOutputQueue(name="depth_out", maxSize=8, blocking=False)
                #self.q_pass_through_depth = self.device.getOutputQueue(name="pass_through_depth", maxSize=8, blocking=False)
    ...
    def create_pipeline(self):
        ...
        if self.input_type == "rgb":
            ...
            if self.xyz:
                ...
                pass_through_depth = pipeline.createXLinkOut()
                pass_through_depth.setStreamName("pass_through_depth")
                pass_through_depth.input.setQueueSize(1)
                pass_through_depth.input.setBlocking(False)
                ...
                spatial_calc_depth_in = pipeline.createXLinkIn()
                spatial_calc_depth_in.setStreamName("spatial_calc_depth_in")
                ...
                #spatial_calc_depth_in.out.link(spatial_location_calculator.inputDepth)
    ...
    def spatial_loc_roi_from_index_landmark(self, hand):
        zone_size = max(int(hand.rect_w_a / 10), 8)        
    ...
    def spatial_loc_roi_from_ith_landmark(self, hand, i):
        zone_size = max(int(hand.rect_w_a / 10), 8)        
        
        rect_center = dai.Point2f(*(hand.landmarks[i]-np.array((zone_size//2 - self.crop_w, zone_size//2 + self.pad_h))))
        rect_size = dai.Size2f(zone_size, zone_size)
        return dai.Rect(rect_center, rect_size)
    ...
    def query_xyz(self, spatial_loc_roi_func):
        ...
        for h in self.hands:
            ...
            conf_data.depthThresholds.lowerThreshold = 100      #focus on the depth from 0.1m to 10m
            conf_data.depthThresholds.upperThreshold = 1000
            conf_data.roi = spatial_loc_roi_func(h, j)
        ...
        if len(conf_datas) > 0:
            ...
            for i, sd in enumerate(spatial_data):
                ...
                coordinates[i, 0] = sd.spatialCoordinates.x
                coordinates[i, 1] = sd.spatialCoordinates.y
                coordinates[i, 2] = sd.spatialCoordinates.z
    ...
    def next_frame(self):
        ...
        if self.input_type == "rgb":
            ...
            #svideo_frame = cv2.flip(video_frame, 1)

            #Original output. Get Stereo.depth and return as depth_frame
            depth_video = self.q_depth_out.get()
            #print(type(depth_video))
            #depth_array = depth_video.getFrame()
            ##左右反転
            #print(depth_array.shape)
            #depth_video.setFrame(depth_array)
            #print(type(depth_video))
            #self.q_spatial_depth_in.send(depth_array)

            #depth_video = self.q_pass_through_depth.get()
            depth_frame = depth_video.getCvFrame()
            #self.q_spatial_depth.send(depth_frame)
            
            #print(type(depth_frame))
            #depth_frame = depth_frame.T#.astype(np.uint16)
        ...
        if self.use_lm:
            ...
            if self.xyz:
                ...
                #self.query_xyz(self.spatial_loc_roi_from_index_landmark)

                coordinates = self.query_xyz(self.spatial_loc_roi_from_ith_landmark)
        ...
        return video_frame, self.hands, bag, depth_frame, coordinates
